...
HELP_TEXT="
  ...
  ${ACCENT}Ctrl + h${RESET}      View app-specific help (fish help)
  ${ACCENT}Ctrl + a${RESET}      Launch with custom tags/arguments
  ${ACCENT}Alt + a${RESET}       Launch with custom tags/arguments
  ${ACCENT}Ctrl + /${RESET}      Show this help menu

  ${BOLD}NAVIGATION${RESET}
  ${ACCENT}Ctrl + n/p${RESET}    Move selection Down / Up
  ${ACCENT}Ctrl + d/u${RESET}    Page Down / Page Up
  ${ACCENT}Ctrl + l${RESET}      Clear search query
  ${ACCENT}Esc / Ctrl+c${RESET}  Quit Launcher
...
MISCELLANEOUS=("anydesk" "appimageupdate" "kando-appimage" "polybar-appimage" "walc" "draw" "astroterm" "wiki-tui" "ttyd" "kitten")
...
selection=$(printf "%s\n" "${MENU[@]}" | fzf \
  ...
  --preview '
    ...
    echo
    echo "'$BOLD'󰧮 Fish file:'$RESET' $file"
  ' \
  --bind "ctrl-/:preview(echo \"$HELP_TEXT\")" \
  --bind "ctrl-h:execute(
      real_fn=\$(echo {} | awk -F '│' '{print \$3}' | xargs);
  ...
  --bind "ctrl-a:become(
      real_fn=\$(echo {} | awk -F '│' '{print \$3}' | xargs);
      read -p \"Add tags/arguments for \$real_fn: \" tags;
      echo \"TAGGED_RUN:\$real_fn \$tags\"
  )" \
  --bind "alt-a:become(
      real_fn=\$(echo {} | awk -F '│' '{print \$3}' | xargs);
      read -p \"Add tags/arguments for \$real_fn: \" tags;
...
# Check if the "Become" action triggered a tagged run
if [[ "$selection" == TAGGED_RUN:* ]]; then
    cmd=${selection#TAGGED_RUN:}
else
